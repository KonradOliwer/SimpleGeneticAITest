...
import pwr.om.geneticalgorithm.bit.GAUtilities;
import pwr.om.geneticalgorithm.bit.Mutation;
...
public class FixedPopulationSizeGeneticAlgorithm {
    ...
    private final BitSet[][] population;
    ...
    public int[] getBest() {
        ...
        return convertPopulation()[bestIndex];
    }

    private int[][] convertPopulation() {
        int[][] result = new int[population.length][];
        for (int i = 0; i < population.length; i++) {
            result[i] = new int[population[i].length];
            for (int j = 0; j < population[i].length; j++) {
                result[i][j] = GAUtilities.convert(population[i][j]);
            }
        }
        return result;
    }
    ...
    public static class Builder {
        ...
        public Builder population(int[][] population) {
            this.population = new BitSet[population.length][];
            for (int i = 0; i < population.length; i++) {
                this.population[i] = new BitSet[population[i].length];
                for (int j = 0; j < population[i].length; j++) {
                    this.population[i][j] = GAUtilities.convert(population[i][j]);
                }
            }
            return this;
        }
